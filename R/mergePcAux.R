...
#'
#' @param pcAuxData An object of class PcAuxData produced by a call to createPcAux.
#' @param rawData A data frame containing the raw data used to run createPcAux.
#' @param nComps A two-element vector giving the number of linear and nonlinear, respectively,
#' component scores to extract. See the Details section for more information.
#' When not specified, all component scores that exist in pcAuxData are used.
#' @param verbose A logical flag indicating whether verbose output should be printed to stdout.
#' Defaults to verbose = TRUE.
#' @param ... Only used when mergePcAux is called from within other PcAux functions.
#'
#' @return A data frame with (a subset of) the principal component auxiliary scores from pcAuxData$pcAux
#' merged onto the end of the raw data.
#' @export
...
mergePcAux <-
           ...
           ...)
  {
    ...
    if (nComps[2] > 0)
      check <-
      check & idVars %in% colnames(pcAuxData$pcAux$nonLin)
