...
flexLinearAssoc <-
  function(varNames,
           ...
           autoType = FALSE)
  {
    withr::local_options(list(warn = -1)) # Suppress warnings
    ...
    if (autoType) {
      ...
      if (any(check))
        varType[check] <- "nominal"

      check <- unlist(lapply(tmp, is.ordered))
      if (any(check))
        varType[check] <- "ordinal"
    } else {
    ...
    withr::local_output_sink(nullFile)# Suppress output

    ## Check for pairwise available obvservations
    pairwiseCheck <- mean(apply(!is.na(map$data[, varNames]), 1, all))
    ...
    if (pairwiseCheck != 0) {
      ...
      if (all(varType == "continuous")) {
        ...
        corVal <- cor(
          as.numeric(map$data[, varNames[1]]),
        ...
        corVal <-
          vcd::assocstats(table(map$data[, varNames]))$cramer
        corType <- "cramer"
      }
      else if (all(varType == "ordinal" |
                   varType == "nominal" |
        ...
        nomVar  <-
          varNames[varType == "nominal" | varType == "binary"]
        contVar <- varNames[varType == "continuous"]
        corVal  <-
          ICC::ICCbare(x = nomVar,
                       y = contVar,
                       data = map$data)
        corType <- "icc"
      }
...
simplePca <- function(map, lv, parse, scale = TRUE)
{
  ...
  map$rSquared[[lv]] <-
    cumsum(eigenOut$values) / sum(eigenOut$values)

  ## Set component counts when some are defined by variance explained:
  if (parse)
    map$setNComps(type = lv)

  nc <- map$nComps[lv]

  ## Compute and save the PcAux scores:
  if (is.null(map$idCols))
    map$pcAux[[lv]] <- data.frame(as.matrix(map$data) %*% eigenOut$vectors[, 1:nc])
  else
    map$pcAux[[lv]] <- data.frame(map$idCols,
...
warnFun <- function(type, map) {
  ...
  warnMessage <-
    switch(
      ...
      highPm =
        paste0(
          "The following variables have fewer than ",
          map$minRespCount,
          " observed responses: ",
      ...
      firstImpFail =
        paste0(
          "First-pass imputation has failed ",
      ...
      pmmFail =
        paste0(
          "Predictive mean matching has failed ",
          "for the following variables: ",
      ...
      groupMeanFail =
        paste0(
          "Group-mean substitution has failed ",
          "for the following variables: ",
      ...
      dropGroupVars =
        paste0(
          "All grouping variables have been dropped, ",
          "so I will employ grand-mean substitution ",
          "to treat the following variables: ",
          toString(map$impFails$pmm),
          ".\n"
      ...
      grandMeanFail =
        paste0(
          "Grand-mean substitution has failed ",
          "for the following variables: ",
      ...
      linPcNum = {
        ...
        nComps <- map$nComps[1]
        paste0(
          "The number of linear principal component ",
      ...
      nonLinPcNum = {
        ...
        nComps <- map$nComps[2]
        paste0(
          "The number of nonlinear principal component ",
          "scores that you requested (i.e., ",
          nComps,
          ") is greater than the number of remaining ",
          "columns in your data object (i.e., ",
      ...
      mergeBadID =
        paste0(
          "None of the potential ID variables (i.e., ",
          toString(map$idVars),
      ...
      miceCrash =
        paste0(
          "The mice() algorithm has crashed while creating ",
          "imputation number ",
          map$impNum,
      ...
      noMods =
        paste0(
          "You have specified 'interactType = ",
          map$intMeth,
      ...
      collinMods = {
        val <- map[, 3]
        paste0(
          "Two of your moderator variables (i.e., ",
          toString(map[, 1:2]),
          ") are ",
  ...
  warning(paste0(c("\r", strwrap(
    warnMessage, width = 81
  )), collapse = "\n"),
  call. = FALSE)
...
errFun <- function(type, ...) {
  ...
  errMessage <-
    switch(
      ...
      nonLinOptionClash =
        paste0(
          "You have requested a non-trivial number of ",
      ...
      missingVars =
        paste0(
          "Some of the arguments you've supplied correspond ",
      ...
      dropVarOverlap =
        paste0(
          "The set of variable names supplied for the ",
      ...
      idOverlap = {
        ...
        paste0(
          "The set of variable names supplied ",
      ...
      miceCrash =
        paste0(
          "The mice() algorithm has crashed ",
          "and returned the following error message:\n",
          x$map$data,
          "\n"
        ),
      badPcaMemLev =
        paste0(
          "An unrecognized value has been specified ",
      ...
      linVarExp =
        paste0(
          "The number of available linear ",
      ...
      fewLinPcAux =
        paste0(
          "The number of linear component scores you ",
          "requested (i.e., ",
          x$nComps[1],
      ...
      nonLinVarExp =
        paste0(
          "The number of available non-linear ",
          "component scores (i.e., ",
      ...
      fewNonLinPcAux =
        paste0(
          "The number of non-linear component scores you ",
          "requested (i.e., ",
          x$nComps[2],
      ...
      catPolyClash = paste0(
        "You have requested polynomial terms up to order ",
        x,
...
makePredMat <- function(map) {
  ...
  if (any(badPredFlag)) {
    ...
    for (v in badVars) {
      ...
      tmp <-
        map$corPairs[map$corPairs$var1 == v |
                       map$corPairs$var2 == v,]
      ...
      if (v %in% candidates) {
        ...
        # 'v' is a polynomial or interaction
        tmp <- strsplit(v, "\\.")[[1]]
