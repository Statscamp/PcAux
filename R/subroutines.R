...
checkInputs <- function() {
  ...
  if (is.null(env$pcAuxData))
    parent <- 2
  else
    parent <- sum(vapply(env$pcAuxData$call,
                         is.null, logical(1)))
  ...
  if (parent == 2) {
    ...
    varNames <-
      with(env, c(idVars, nomVars, ordVars, groupVars, dropVars))
  ...
  if (parent == 1) {
    ...
    if (env$maxPolyPow < 1)
      errFun("smallPower")
    else if (env$maxPolyPow > 4)
      errFun("largePower")

    ## Check for non-zero linear component counts:
    if (env$nComps[1] == 0)
      errFun("noLinPcAux", creatingPcAux = TRUE)

    ## Check for disagreement between nComps and usePoly/useInteract:
    checkVal <-
      env$interactType == 0 &
      env$maxPolyPow == 1 & env$nComps[2] > 0
    if (checkVal)
      errFun("nonLinOptionClash", nNonLinear = env$nComps[2])
...
castData <- function(map) {
  ...
  creatingPcAux <-
    length(map$time$create) > 0 &&
    length(map$time$mi) == 0 # Are we in createPcAux()?
  ...
  if (confirmTypes) {
    ...
    if (length(map$probNoms) > 0) {
      # Any suspicious nominal variables?
      warnFun("badNoms", map)
    ...
    if (length(map$probOrds) > 0) {
      # Any suspicious ordinal variables?
      warnFun("badOrds", map)
    ...
    if (length(map$probCons) > 0) {
      # Any suspicious continuous variables?
      warnFun("badCons", map)
...
cleanData <- function(map) {
  ...
  creatingPcAux <-
    length(map$pcAux$lin) == 0 # Are we in createPcAux()?

  if (creatingPcAux) {
    ## Check for factor id columns
    checkFactors <- vapply(map$idCols, is.factor, logical(1))
    if (any(checkFactors))
      map$idCols[checkFactors] <-
        lapply(map$idCols[checkFactors], as.character)
    if (length(map$idVars) > 1) {
      ...
      if (length(missIds) > 1) {
        # More than 1 incomplete ID
        map$idFills <-
          lapply(map$idCols, FUN = createDummyIdValues)

        ## Fill missing IDs with their dummy values
        for (i in missIds)
          map$idCols[, i][is.na(map$idCols[, i])] <-
            map$idFills[[i]]

      } else if (length(missIds) == 1) {
        # Only 1 incomplete ID
  ...
  if (map$verbose > 0)
    cat("--Checking for empty columns...")
  ...
  if (map$verbose > 0)
    cat("--Checking for high PM...")
  haveHighPmVars <- map$findHighPmVars()
  if (map$verbose > 0)
    cat("done.\n")

  if (haveHighPmVars) {
    # Any low-response variables?
    warnFun("highPm", map)
    ...
    if (length(ansCheck) == 0) {
      # Remove the low-response variables
  ...
  if (haveEmptyVars)
    warnFun("emptyVars", map)

  if (haveConstCols)
    warnFun(ifelse(creatingPcAux, "pcAuxConstCols", "romConstCols"),
            map)
...
doSingleImputation <- function(map, micemethods = micemethods) {
  ...
  if (any(map$respCounts > 0)) {
    ...
    map$updateImpFails(colnames(map$data)[map$respCounts > 0], type = "grandMean")
...
meanSubstitute <- function(map) {
  ...
  if (missCheck(map$groupVars)) {
    # All groupVars have been dropped
  ...
  if (any(map$respCounts > 0)) {
    ...
    map$updateImpFails(colnames(map$data)[map$respCounts > 0], "groupMean")
...
doPCA <- function(map) {
  ...
  if (linVal == "lin") {
    ...
    if (!missCheck(map$ordVars))
      map$castOrdVars()
    ## Dummy code nominal variables:
    if (!missCheck(map$nomVars))
      map$castNomVars()
    ...
    if (sum(check) > 1)
      map$data[, check] <-
      data.frame(lapply(map$data[, check], f2n))
    if (sum(check) == 1)
      ...
      map$poly <- data.frame(lapply(map$poly,
                                    ...
                                    pc = with(map,
                                              pcAux$lin[, setdiff(colnames(pcAux$lin),
                                                                  idVars)])))
      if (map$intMeth > 1)
        map$data <- data.frame(map$data, map$poly)
      else
        map$data <- map$poly
    }
    colnames(map$data) <-
      c(colnames(map$interact), colnames(map$poly))
  ...
  if (map$pcaMemLev == 0) {
    ...
    map$rSquared[[linVal]] <-
      cumsum(pcaOut$sdev ^ 2) / sum(pcaOut$sdev ^ 2)

    ## Set component counts when some are defined in terms of variance
    ## explained:
    if (parseCheck)
      map$setNComps(type = pcType)
  ...
  if (linVal == "nonLin")
    map$data <- "Removed to save resources"

  ## Give some informative column names:
  colnames(map$pcAux[[linVal]]) <-
    c(map$idVars,
      paste0(
        ifelse(linVal == "lin", "linPC", "nonLinPC"),
...
parallelMice <- function(imp, map, tempDirName) {
  ## Setup the PRNG:
  tmpSeed <-
    ifelse(is.na(map$seed), round(runif(1, 1, 1e6)), map$seed)
  ...
  if (class(miceOut) != "try-error") {
    ...
    if (imp == 1)
      saveRDS(object = miceOut,
              file = file.path(tempDirName, "firstMids.rds"))
  } else {
    impData <- NULL
    warnFun("miceCrash",
