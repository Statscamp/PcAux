...
PcAuxData$methods(
                        ...
                        minPcPredCount = 1L)
  {
    ...
    ## Structured fields:
    call <<- list(prepData    = NULL,
                  createPcAux = NULL,
                  miWithPcAux = NULL)
    pcAux <<- list(lin    = data.frame(NULL),
                   nonLin = data.frame(NULL))
    rSquared <<- list(lin    = vector("numeric"),
  ...
  setCall = function(x, parent)
  {
    if (parent == "prepData")
      call[[1]] <<- x
    else if (parent == "createPcAux")
      call[[2]] <<- x
    else if (parent == "miWithPcAux")
      call[[3]] <<- x
    else
  ...
  setNComps = function(type)
  {
    ...
    if (is.infinite(nc)) {
      ...
      nComps[type] <<-
        ifelse(length(tmp) == 0, length(r2), tmp[1])
    } else if (nc < 1 & nc > 0) {
  ...
  setStatus = function(step = "start")
  {
    ...
    stCall <-
      sum(vapply(call, function(x)
        is.null(x), logical(1)))

    if (stCall == 2)
      status$prep[[step]]   <<- session
    else if (stCall == 1)
      status$create[[step]] <<- session
    else if (stCall == 0)
      status$mi[[step]]     <<- session
  },
  ...
  setTime = function(step = "start")
  {
    ...
    stCall <-
      sum(vapply(call, function(x)
        is.null(x), logical(1)))
    if (stCall == 2)
      time$prep[[step]] <<- proc.time()["elapsed"]
    else if (stCall == 1)
      time$create[step] <<- proc.time()["elapsed"]
    else if (stCall == 0)
      time$mi[step]     <<- proc.time()["elapsed"]
  },
  ...
  removeVars = function(x, reason, recordOnly = FALSE)
  {
    ...
    dropVars <<-
      rbind(dropVars, cbind(colnames(data)[dropCols], reason))
    if (!recordOnly)
      data <<- data[,-dropCols]
  },
  ...
  countVarLevels = function()
  {
    ...
    levelVec <<- as.integer(unlist(lapply(data, function(x)
      length(unique(
        na.omit(x)
      )))))
  ...
  checkTypes = function()
  {
    ...
    if (length(tmpN) > 0)
      probNoms <<- names(levelVec)[tmpN]
    if (length(tmpO) > 0)
      probOrds <<- names(levelVec)[tmpO]
    if (length(tmpC) > 0)
      probCons <<- names(levelVec)[tmpC]
  },
                            ...
                            initialPm    = FALSE)
  {
    ...
    if (strict & noMissing)
      stop(
        paste0(
          "The data provided are completely ",
  ...
  findHighPmVars = function()
  {
    ...
    tmpNames   <-
      setdiff   (names(respCounts), dropVars[, 1])
    tmpCounts  <-
      respCounts[tmpNames]
    highPmVars <<-
      tmpNames  [tmpCounts > 0 & tmpCounts < minRespCount]
  ...
  findEmptyVars = function(remove = TRUE)
  {
    ...
    if (length(emptyVars) > 0) {
      # Find any empty variables?
      removeVars(x = emptyVars,
                 reason = "empty",
                 recordOnly = !remove)
      outFlag <- TRUE
    } else {
  ...
  findConstCols = function()
  {
    ...
    creatingPcAux <-
      length(pcAux$lin) == 0 # Are we in createPcAux()?
    tmpNames      <-  setdiff (names(typeVec), dropVars[, 1])
    tmpTypes      <-  typeVec [tmpNames]
  ...
  fillNomCell = function(name)
  {
    ...
    for (n in name) {
      tmp         <- table(data[, n])
      impVal      <-
        sample(levels(data[, n]), size = 1, prob = tmp)
  ...
  createPatterns = function()
  {
    ...
    patterns[[length(groupVars)]] <<-
      format(data[, groupVars[1]])

    if (gVarCheck)
      binGroupVars(undo = TRUE)
  },
  ...
  completeMiData = function()
  {
    ...
    if (specialComp) {
      ...
      for (m in 1:nImps) {
        ...
        if (m == 1)
          pcCols <- grep("^linPC\\d|^nonLinPC\\d",
  ...
  transformMiData = function()
  {
    ...
    ## Remove the PcAux:
    pcCols <-
      grep("^linPC\\d|^nonLinPC\\d", colnames(miDatasets[[1]]))
    for (m in 1:nImps)
      miDatasets[[m]] <<- miDatasets[[m]][,-pcCols]
  ...
  computeInteract = function()
  {
    ...
    if (length(pcAux$lin) > 0)
      # Do we have linear PcAux?
    ...
    if (intMeth == 1)
      focal <- setdiff(colnames(data), mods)
    else
      focal <- pcNames
    ...
    intVars <<-
      as.character(sapply(varCombs, paste0, collapse = "."))

    ## Compute the interaction terms:
    if (intMeth == 1)
      interact <<- data.frame(lapply(varCombs,
                                     function(x, dat)
                                       dat[, x[1]] * dat[, x[2]],
                                     dat = data))
    else
    ...
    if (length(ordVars) > 0)
      castOrdVars(toNumeric = FALSE)
    if (length(nomVars) > 0)
      castNomVars(toNumeric = FALSE)
  },
  ...
  computePoly = function()
  {
    ...
    if (length(dataNames) == 1)
      # Hack for only one target variable
      poly <<- data.frame(model.matrix(form,
                                       data = as.data.frame(list(data[, dataNames]),
                                                            col.names = dataNames))[,-1])
    else
  ...
  codeNomVars = function()
  {
    ...
    for (v in noms) {
      ...
      levVec                 <-
        vapply(tmp, countLevels, integer(1))
